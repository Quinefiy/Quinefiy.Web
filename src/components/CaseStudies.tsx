...
const CaseStudies = ({ showHeader }: Props) => {
    const cases = [
        {
            id: 1,
            img: "",
            title: "Case Study Title",
            desc: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliqui ggggg kkkkk"
        },
        {
            id: 2,
            img: "",
        ...
        <div className="!w-full ">
            {showHeader &&
                <div className="flex flex-col items-center gap-[24px] 2xl:mb-[64px] xl:mb-[58px] lg:mb-[50px] md:mb-[44px] sm:mb-[38px] mb-[32px]">
                    ...
                    <h2 className="opacity-0 animate-down-on-scroll  text-[#E1E1E1] 2xl:text-[36px] xl:text-[32px] lg:text-[28px] md:text-[24px] sm:text-[20px] text-[18px] font-bold leading-[160%]">Our Case Studies</h2>
                </div>
            }
            <div className="w-full flex justify-center items-center gap-[24px] flex-wrap ">
                {cases.map((c: any,index: number) => (
                    <div data-delay={index * 0.3} className="sm:w-[33%] w-full max-w-[420px] sm:min-w-[408px] priceCard-gradient rounded-[16px] lg:p-[32px] p-[24px] border border-[#6BAAFF4D] hover:border-[#00429B80] flex flex-col gap-[32px] opacity-0 animate-down-on-scroll" key={c.id}>
                        <img className="w-full h-[224px] rounded-[12px]" />
                        <div>
                            <h4 className="font-bold text-[#E1E1E1] leading-[140%] lg:text-[18px] text-[16px] mb-[16px]">{c.title}</h4>
                            <p className="font-medium text-[#FFFFFFB2] leading-[140%] lg:text-[16px] text-[14px] line-clamp-5">{c.desc}</p>
                        </div>
