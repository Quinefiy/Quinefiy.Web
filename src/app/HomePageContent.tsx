...
export default function HomePageContent() {
  ...
  const { data: heroData, isLoading: loadingHeroData } = useQuery({
    queryKey: ["hero-data"],
    queryFn: getHeroSectionData,
  });
  const { data: softwareProducts, isLoading: loadingSoftwareProducts } = useQuery({
    queryKey: ["software-products"],
    queryFn: getProducts,
  });
  const { data: pricingPlans, isLoading: loadingPricing } = useQuery({
    queryKey: ["pricing-palns"],
    queryFn: getPricingPlans,
  });
  const { data: caseStudies, isLoading: loadingCases } = useQuery({
    queryKey: ["cases-studies"],
  ...
  const content = (
    <div className="w-full flex flex-col xl:gap-[150px] md:gap-[120px] gap-[80px] items-center 2xl:px-[96px] xl:px-[52px] lg:px-[36px] md:px-[21px] sm:px-[19px] px-[16px] ">
      ...
      <div className="w-full max-w-[1535px] py-[30px] flex flex-col items-center 2xl:gap-[60px] xl:gap-[50px] lg:gap-[40px] md:gap-[32px] sm:gap-[24px] gap-[16px]">
        <h3 className="text-[#999999] font-semibold xl:text-[18px] md:text-[17px] sm:text-[15px] text-[13px]">
  ...
  // If already loaded before → no loader, just content
  if (hasLoadedBefore) {
    return content;
  }

  // First time → wrap with loader
